			...
			<v-card-text class="px-0 py-0">
				<v-tabs fixed-tabs v-model="activeTab">
					...
					<v-tab-item key="timer">
						<v-time-picker ref="timer" class="v-time-picker-custom" v-model="time" v-bind="timePickerProps" :format="format" use-seconds full-width></v-time-picker>
					</v-tab-item>
				</v-tabs>
			</v-card-text>
			<v-card-actions>
...
<script>
	...
	const DEFAULT_FORMAT = '24hr'
	const DEFAULT_DATE_FORMAT = 'yyyy-MM-dd'
	const DEFAULT_TIME_FORMAT = 'HH:mm:ss'
	const DEFAULT_DIALOG_WIDTH = 340
	const DEFAULT_CLEAR_TEXT = 'Отмена'
	const DEFAULT_OK_TEXT = 'Открыть'
	...
	export default {
		...
		methods: {
			...
			okHandler() {
				this.resetPicker()
				this.$root.$emit('okCalendar', [this.date, this.time]);
			},
			clearHandler() {
				this.resetPicker()
